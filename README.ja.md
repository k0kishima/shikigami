...
- [Claude Code](https://claude.ai/code) >= 2.1.32（Agent Teams 機能が必要）
  - ネイティブインストーラーの利用を推奨します。npm 版（特に Volta 等のバージョンマネージャー経由）ではバージョン管理の問題が発生する場合があります。
  - `curl -fsSL https://claude.ai/install.sh | sh`
  - 詳細は [Getting Started](https://docs.anthropic.com/en/docs/claude-code/getting-started) を参照してください。
- macOS または Linux（UNIX 系 OS）
...
インストール後、ターミナルを再起動するか `source ~/.zshrc`（または `~/.bashrc`）を実行してください。
...
オーケストレーターが起動し、要件分析のプロセスをガイドします。
...
- [カスタムロール](./docs/custom-roles.ja.md)
