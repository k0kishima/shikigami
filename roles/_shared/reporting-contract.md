...
> **Note on the lead bus name**: In Shikigami, the Orchestrator is addressable as `team-lead`. This is the established convention used throughout worker role templates. When sending a `SendMessage` back to the Orchestrator, use `to="team-lead"`. This is the Claude Code Agent Teams runtime default for the team lead's `leadAgentId`.

## Required signals

- **On start**: Immediately after receiving the task, send a one-line `SendMessage(to="team-lead")` acknowledging you've started. Include the high-level plan in one sentence.
- **On blockers**: If you hit any blocker, send `SendMessage(to="team-lead")` immediately describing what is blocking you. Do NOT silently wait or self-resolve based on assumptions that weren't in the spec.
...
- **On completion**: Send a `SendMessage(to="team-lead")` with the deliverable summary. This is mandatory even if the work was trivial.
- **Silence is failure**: If you cannot make progress for any reason, report it. Never leave the `team-lead` waiting without a signal.
